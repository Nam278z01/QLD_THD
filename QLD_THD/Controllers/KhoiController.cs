...
namespace QLD_THD.Controllers
{
    ...
    public class KhoiController : ControllerBase
    {
        ...
        IKhoiBusiness _bus;
        public KhoiController(IKhoiBusiness bus, IWebHostEnvironment env)
        {
        ...
        [Route("search")]
        [HttpPost]
        public async Task<ResponseListMessage<List<Khoi>>> Search([FromBody] Dictionary<string, object> formData)
        {
            var response = new ResponseListMessage<List<Khoi>>();
            try
            {
                long total = 0;
                var page = int.Parse(formData["page"].ToString());
                var pageSize = int.Parse(formData["pageSize"].ToString());
                var mamh = formData.Keys.Contains("makh") ? Convert.ToString(formData["makh"]) : "";
                var tenmh = formData.Keys.Contains("tenkh") ? Convert.ToString(formData["tenkh"]) : "";
        ...
        [Route("get-by-id/{id}")]
        [HttpGet]
        public async Task<ResponseMessage<Khoi>> GetById(string id)
        {
            var response = new ResponseMessage<Khoi>();
            try
            {
        ...
        [Route("create")]
        [HttpPost]
        public async Task<ResponseMessage<Khoi>> Create([FromBody] Khoi model)
        {
            var response = new ResponseMessage<Khoi>();
            try
            {
        ...
        [Route("update")]
        [HttpPost]
        public async Task<ResponseMessage<Khoi>> UpdateWebsiteTag([FromBody] Khoi model)
        {
            var response = new ResponseMessage<Khoi>();
            try
            {
