...
namespace QLD_THD.Controllers
{
    ...
    public class HocSinhController : ControllerBase
    {
        ...
        public HocSinhController(IHocSinhBusiness bus, IWebHostEnvironment env)
        {
        ...
        [Route("search")]
        [HttpPost]
        public async Task<ResponseListMessage<List<HocSinh>>> Search([FromBody] Dictionary<string, object> formData)
        {
            var response = new ResponseListMessage<List<HocSinh>>();
            try
            {
                long total = 0;
        ...
        [Route("get-by-id/{id}")]
        [HttpGet]
        public async Task<ResponseMessage<HocSinh>> GetById(string id)
        {
            var response = new ResponseMessage<HocSinh>();
            try
            {
        ...
        [Route("create")]
        [HttpPost]
        public async Task<ResponseMessage<HocSinh>> Create([FromBody] HocSinh model)
        {
            var response = new ResponseMessage<HocSinh>();
            try
            {
        ...
        [Route("update")]
        [HttpPost]
        public async Task<ResponseMessage<HocSinh>> UpdateWebsiteTag([FromBody] HocSinh model)
        {
            var response = new ResponseMessage<HocSinh>();
            try
            {
        ...
        public async Task<IActionResult> Upload(IFormFile file)
        {
            try
            {
                if (file.Length > 0 && file.FileName.Contains(".xlsx"))
                {
                    ...
                    var list = ExcelHelper.ConvertDataTable<HocSinh>(data);
                    foreach (var model in list)
                        if (!string.IsNullOrEmpty(model.MaHocSinh))
                        {
        ...
        public async Task<IActionResult> ExportToExcel([FromBody] Dictionary<string, object> formData)
        {
            try
            {
                ...
                dataExport.Columns.Add("MaHocSinh");
                dataExport.Columns.Add("TenHocSinh");
                dataExport.Columns.Add("TenLop");
                ...
                if (data.Count > 0)
                {
                    foreach (var item in data)
                    {
                        ...
                        row["MaHocSinh"] = item.MaHocSinh;
                        row["TenHocSinh"] = item.HoTen;
                        row["TenLop"] = item.MaLop;
